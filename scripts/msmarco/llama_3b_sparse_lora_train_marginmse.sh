...
list_of_tuples=(
    1e-4 '[1.,.05,.04]' 128 512 24
)
NGPU=4
# 4 A100, ep=8, bs=64, ef_bs=512 takes 40hours - 48hours (roughly 2 days) for llama3-8b 
...
for (( i=0; i<${#list_of_tuples[@]}; i+=3 )); do
    ...
    run_name=llama3-3b-marco-mntp-sparse-mgmse-lora-${lr}_qreg_${query_reg}_dreg_${doc_reg}_bs_${batch_size}_ep_${epochs}
