...
os.environ["WANDB_PROJECT"] = "llm_as_retriever"
require_version(
    "datasets>=1.8.0",
...
class DataTrainingArguments:
    ...
    validation_split_length: Optional[int] = field(
        default=1000,
        metadata={
            "help": "The number of samples to use as validation set in case there's no validation split"
        },
    )
    ...
    def __post_init__(self):
        ...
        if (
            ...
            if self.train_file is not None:
                ...
                if extension not in ["csv", "json", "txt", "tsv"]:
                    raise ValueError(
...
class MNTPTrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        print("save model type: ", type(self.model))
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        ################# Hansi ###################
        if extension == "tsv":
            # This is used for loading the msmarco corpus endswith .tsv 
            print("Loading the dataset from the tsv file", data_files["train"])
            raw_datasets = load_dataset(
                "csv",
                data_files=data_files,
                delimiter='\t',
                column_names=['docid', 'text'],
                cache_dir=model_args.cache_dir,
                token=model_args.token,
            )
        else:
    ...
    ##################### Hansi #################
    # original comment: model organization is MODEL_TYPEBiForMNTP.model -> MODEL_TYPELBiModel, we have to apply PEFT to the inner model
    # However, Our Spalde model directly use MNTP model as the base model, hence PEFT should also wrap the MNTP model.
