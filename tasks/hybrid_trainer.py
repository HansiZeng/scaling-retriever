...
class HybridTrainingArgs(TrainingArguments):
    ...
    task_weights: str = field(default=None)
    ...
    def __post_init__(self):
        ...
        self.ln_to_weight = {}
        self.task_weights = eval(self.task_weights)
        self.task_names = eval(self.task_names)
        assert len(self.task_names) == len(self.task_weights)
        for ln, weight in zip(self.task_names, self.task_weights):
            self.ln_to_weight[ln] = weight
            
        self.num_tasks = len(self.task_names)
        print("task_names", self.task_names, self.ln_to_weight)
        
        if self.lora_modules_to_save is not None:
            self.lora_modules_to_save = eval(self.lora_modules_to_save)
            print("lora_modules_to_save: ", self.lora_modules_to_save)
            
        assert self.loss_type in ["nce", "margin_mse"]
        
        if self.loss_type == "nce":
            assert self.task_names == ["lex_rank", "dense_rank", "hybrid_rank", "query_reg", "doc_reg"]
        elif self.loss_type == "margin_mse":
            assert self.task_names == ["lex_rank", "dense_rank", "query_reg", "doc_reg"]
...
class HybridTrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]):
        ...
        return loss.detach() / self.args.gradient_accumulation_steps, \
            {k: v / self.args.gradient_accumulation_steps for k, v in task_losses.items()}
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if isinstance(tr_task_losses_step, dict): ###################
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_task_losses, grad_norm, model, trial, epoch, ignore_keys_for_eval):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if self.is_fsdp_enabled:
            if isinstance(self.model, PreTrainedModel):
                print("model is PreTrainedModel")
                unwrap_model(self.model).save_pretrained(output_dir, state_dict=state_dict) if state_dict is not None else \
                    unwrap_model(self.model).save_pretrained(output_dir)
            else:
                print("Not PreTrainedModel")
                ...
                if isinstance(self.model.base_model, PeftModel):
                    lora_state_dict = get_peft_model_state_dict(self.model.base_model, state_dict)
                    
                if self.accelerator.is_main_process:
                    if lora_state_dict is not None:
                        torch.save(lora_state_dict, os.path.join(output_dir, "adapter_model.bin"))
                        self.model.base_model.peft_config["default"].save_pretrained(output_dir)
