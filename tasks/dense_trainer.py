...
@dataclass
class DenseTrainingArgs(TrainingArguments):
    ...
    T: float = field(default=0.01) # this is the temperature
    ...
    def __post_init__(self):
        ...
        assert self.loss_type in ["nce", "margin_mse", "kldiv", "nce_kldiv"]
...
class DenseTrainerForNCE_KLdiv(DenseTrainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        losses = model(**inputs)
        return losses
    ...
    def _inner_training_loop(
        ...
        tr_task_losses = torch.zeros(2).to(args.device) ################
        
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        self._total_task_loss_scalars = [0.0 for _ in range(2)] ################
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if isinstance(tr_task_losses_step, dict): ###################
                        for i, ln in enumerate(["nce", "kldiv"]): ###################
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_task_losses, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            for k, l in zip(["nce", "kldiv"], log_losses):
                logs[f"{k}_loss"] = l
            ############################################
